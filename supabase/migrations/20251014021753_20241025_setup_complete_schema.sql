/*
  # Complete Schema Setup for Valora Plus
  
  1. Tables
    - profiles: User profiles with roles
    - workshops: Workshop information  
    - analysis: PDF analysis tracking
    - vehicle_data: Extracted vehicle information
    - insurance_amounts: Extracted financial data
...
CREATE POLICY "Users can view their own vehicle data" ON vehicle_data
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM analysis WHERE analysis.id = vehicle_data.analysis_id AND analysis.user_id = auth.uid())
  );
...
CREATE POLICY "Users can view their own insurance amounts" ON insurance_amounts
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM analysis WHERE analysis.id = insurance_amounts.analysis_id AND analysis.user_id = auth.uid())
  );
